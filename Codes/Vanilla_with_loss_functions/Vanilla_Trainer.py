...
class Trainer:
    def __init__(self, model, dataloader, epochs, num_classes, lr, device, loss, log_dir):
        ...
        with open(self.log_file, 'w') as f:
            f.write("Epoch, Loss, Dice Score, Dice Loss, CE Loss, Additional Loss, Time\n")
    ...
    def train(self):
        for epoch in range(1, self.epochs + 1):
            ...
            epoch_additional_loss /= len(self.dataloader)
            ...
            print(f"Epoch {epoch}: Loss={epoch_loss:.4f}, Dice Score={epoch_dice:.4f}, Dice Loss={epoch_dice_loss:.4f}, CE Loss={epoch_ce_loss:.4f}, Additional Loss={epoch_additional_loss:.4f}, Time={epoch_time:.2f}s")
            with open(self.log_file, 'a') as f:
                # f.write(f"{epoch}, {epoch_loss:.4f}, {epoch_dice:.4f}, {epoch_time:.2f}\n")
                f.write(f"{epoch}, {epoch_loss:.4f}, {epoch_dice:.4f}, {epoch_dice_loss:.4f}, {epoch_ce_loss:.4f}, {epoch_additional_loss:.4f}, {epoch_time:.2f}\n")
