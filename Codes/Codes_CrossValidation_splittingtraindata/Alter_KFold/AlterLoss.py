...
class AlterLoss(nn.Module):
    ...
    def update_mask(self, pred_mask, gt_mask):
        ...
        return torch.tensor(updated_masks, dtype=pred_mask.dtype, device=pred_mask.device)
...
class NDLoss(nn.Module):
    def __init__(self, num_samples, num_classes, batch_size, device, epsilon=1e-6):
