...
path_to_processed_masks_dir = os.path.join(dest_dir,"processed_masks")
os.makedirs(path_to_processed_masks_dir, exist_ok=True)
for mask_name in sorted(os.listdir(source_masks_dir)):
...
for fname in image_filenames:
    ...
    processed_mask_path = os.path.join(path_to_processed_masks_dir, fname.replace("_0000",""))
    
    if os.path.exists(mask_path):
        data_pairs.append({
            "image": image_path,
            "mask": mask_path,
            "processed_mask": processed_mask_path
        })
    else:
        print(mask_path)
