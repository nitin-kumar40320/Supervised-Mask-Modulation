...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        '''
        Inputs : preds, gt_mask - Tensors of shape [B, C, H, W, D]
        '''
        assert targets.ndim == 5, f"Expected 4D tensor for targets (B, C, H, W, D) in DiceLoss, but got shape {targets.shape}"
        ...
        dims = list(range(2, targets.ndim))  # sum over batch, height, width
...
def DiceScore(preds, targets, epsilon=1e-6):
    ...
    preds: Tensor of shape [B, C, H, W, D] (logits)
    targets: Tensor of shape [B, 1, H, W] (class indices) or [B, C, H, W, D] (one-hot vectors)
    """
    # print('Dice Score Calc')
    # Apply softmax to get class probabilities
    preds = torch.softmax(preds.float(), dim=1)  # [B, C, H, W, D]
    # print(f'Predictions shape : {preds.shape} -- DiceScore')
    # print(f'Targets Shape : {targets.shape} -- DiceScore')
    if targets.shape[1] == 1:
        targets = targets.squeeze(1)
    targets = F.one_hot(targets, num_classes=preds.shape[1]).permute(0, -1, *range(1, targets.ndim)).float()
    # print(f'One-Hot Targets Shape : {targets.shape} -- DiceScore')
    # Compute intersection and union
    dims = list(range(2, targets.ndim))  # Sum over height, width
    intersection = torch.sum(preds * targets, dims)
    union = preds.sum(dim=dims) + targets.sum(dim=dims)
