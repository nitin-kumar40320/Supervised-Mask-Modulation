...
color_list = [
    ...
    (0.75, 0.75, 0.75)   # lighter gray
]
custom_cmap = mcolors.ListedColormap(color_list)

class Trainer:
    def __init__(self, model, dataloader, epochs, num_classes, lr, device, loss, log_dir, imagenet_mean=None, imagenet_std=None):
        ...
        self.latest_checkpoint_path = os.path.join(log_dir, "latest_checkpoint.pth")
        self.best_checkpoint_path = os.path.join(log_dir, "best_checkpoint.pth")
        self.best_dice = 0.0
        self.train_losses = []
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch in self.dataloader:
                ...
                loss = loss_tuple[0]
                dice_loss = loss_tuple[1].detach().cpu().item()
                ce_loss = loss_tuple[2].detach().cpu().item()
                ...
                # output_path = os.path.join(self.img_save_dir, f"{epoch}epoch_{mask_names}.png")
            ...
            # logging.info(f"Epoch {epoch}: Loss={epoch_loss:.4f}, Dice Score={epoch_dice:.4f}, Dice Loss={epoch_dice_loss:.4f}, CE Loss={epoch_ce_loss:.4f}, Time={epoch_time:.2f}s")
    ...
    def plot_metrics(self):
        """
        Plots the training metrics (loss and dice score) over epochs.
        """
